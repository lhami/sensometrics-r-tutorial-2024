...
```{r filtering down to rows matching a criteria}
raw_berry_data %>%
...
```{r filtering rows that are one of a set of options}
raw_berry_data %>%
...
```{r stringr functions as filter helpers}
raw_berry_data %>%
  distinct(test_day)
...
```{r creating new columns with mutate}
berry_columns %>%
...
```{r rescaling the liking scores using mutate}
berry_columns %>%
  mutate(lms_overall = (lms_overall + 100) * (8 / 200) + 1,
         us_overall = (us_overall + 0) * (8 / 15) + 1) %>%
...
```{r saving the results of mutate to the same name}
...
```{r previewing our raw cider data}
raw_cider_data
```
...
```{r pivoting the cider data to a longer format}
long_cider_data <- 
  raw_cider_data %>%
...
```{r pivoting the berry data to a longer format}
long_berry_data <- 
  berry_columns %>%
...
```{r pivoting the berry data to a yet longer format}
long_berry_data <- 
  long_berry_data %>%
  pivot_longer(starts_with("cata_"),
               names_to = "cata_variable",
...
```{r pivoting the long cider data wider again}
long_cider_data %>%
...
```{r penalty analysis summary statistics for cider data}
cider_penalty_data <- 
  long_cider_data %>%
  group_by(cata_variable, checked) %>%
  summarize(rating = mean(Liking),
...
```{r penalty analysis summary statistics for berry data}
berry_penalty_data <- 
  long_berry_data %>%
  group_by(berry, cata_variable, checked) %>%
  summarize(penalty_lift = mean(rating),
...
```{r renaming columns by position, message = FALSE}
read_csv("data/CiderDryness_SensoryDATA.csv") %>%
...
```{r rearranging rows descending with ties broken}
cider_penalty_data %>%
