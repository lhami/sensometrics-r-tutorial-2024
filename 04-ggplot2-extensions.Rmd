...
```{r setup-4, include = FALSE, purl = FALSE, echo = FALSE}
library(tidyverse)
library(ca)
library(ggrepel)
library(patchwork)
...
```{r geom_text_repel text borders, message = FALSE}
raw_cider_data %>%
...
```{r combining plots with patchwork, warning = FALSE}
library(patchwork)
...
```{r arranging plots vertically with patchwork, warning = FALSE}
plot(berry_mfa_res, choix = "var") / plot(berry_mfa_res, partial = "all")
```
...
```{r collecting legends at the bottom of a patchwork ensemble, warning = FALSE}
plot(berry_mfa_res, choix = "var") + plot(berry_mfa_res, partial = "all") +
...
```{r more complex patchwork layout, warning = FALSE}
...
```{r plotting a list, warning = FALSE}
...
```{r labeling plots, warning = FALSE}
plot(berry_mfa_res, choix = "var") + plot(berry_mfa_res, partial = "all") +
  plot_layout(guides = "collect") +
...
```{r moving plot labels, warning = FALSE}
