...
```{r}
#Normally, we present CA results as a graph, and you can use plot() directly
#on the results for quick results:
plot(berry_cata_ca)

#But if you want to make this look publication-friendly by getting rid of overlapping text,
#changing font size and color, color-coding your berries, etc, it's much easier to use
#the tidyverse package ggplot2 to manipulate the graph than using "base R" plotting.
#Since ggplot2 is a tidyverse package, we'll need to re-tidy the data.

#ggplot2 generally works best when you have one row for every observation or point,
#By default, the scatterplot function needs one row per point with one column of
#coordinates for each axis (normally x and y):
...
#and the bar plot/histogram functions are given a categorical or binnable numeric variable,
#respectively, with one row per observation to *count*. These don't draw one 
#although you can override them.
...
#Note: there are exceptions! You may eventually end up having to do something like this:
...
#So we need to think about what we want to plot. The CA maps are **scatterplots**
#where each point is a **sample** (row variable) or **attribute** (column variable).
#These coordinates are in the list that the ca function outputs, but in two separate
#matrices. We want to combine them into *one* where the columns are *axes* and the
#rows are the *variables* from our initial dataset. It's also good to have a column
#specifying whether each row is a sample or an attribute variable.

#So we can simply **retidy** and **row bind** the two coordinate matrices we get from
#ca():

bind_rows(Berry = rownames_to_column(as.data.frame(berry_cata_ca$rowcoord), "Variable"),
          Attribute = rownames_to_column(as.data.frame(berry_cata_ca$colcoord), "Variable"),
          .id = "Type") %>%
  as_tibble() -> berry_ca_std_coords_tidy
